...
import { Plus, Search, Filter, Phone, Mail, MapPin, Eye, FileText, ArrowUpDown, ChevronRight } from 'lucide-react'
...
export function CustomersPage() {
  ...
  const fetchCustomers = async () => {
    try {
      setLoading(true)
      const data = await partiesApi.getParties({ type: 'CUSTOMER', search: search || undefined })
      setCustomers(data)
    } catch (err) {
  ...
  const totalToCollect = customers.reduce((sum, c) => (c.currentBalance > 0 ? sum + c.currentBalance : sum), 0)
  const totalToPay = customers.reduce((sum, c) => (c.currentBalance < 0 ? sum + Math.abs(c.currentBalance) : sum), 0)
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Manage customer accounts, ledger balances, and payment terms
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
          ...
          <div className="text-2xl font-bold text-slate-900 mt-1.5">{customers.length}</div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
          <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total To Collect (Receivable)</span>
          <div className="text-2xl font-bold text-amber-600 mt-1.5">{formatPaise(totalToCollect)}</div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
          <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Advance / Excess (Payable)</span>
          <div className="text-2xl font-bold text-emerald-600 mt-1.5">{formatPaise(totalToPay)}</div>
        </div>
      </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {customers.map((c) => (
                  <tr key={c.id} className="hover:bg-slate-50/75 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-slate-600">
                      {c.gstin ? (
                        <span className="font-mono text-slate-700 font-medium">{c.gstin}</span>
                      ) : (
                        <span className="text-slate-400 italic">Unregistered</span>
                      )}
                      <div className="text-[11px] text-slate-400">{c.billingAddress?.city || '—'}</div>
                    </td>
                    <td className="py-3.5 px-4 text-slate-600">
                      {c.creditPeriodDays ? `${c.creditPeriodDays} Days` : 'Immediate'}
                    </td>
                    <td className="py-3.5 px-5 text-right">
                      ...
                      >
                        {formatPaise(Math.abs(c.currentBalance))}
                      </div>
                      <div className="text-[10px] text-slate-400">
                        {c.currentBalance > 0 ? 'To Collect' : c.currentBalance < 0 ? 'Advance' : 'Settled'}
                      </div>
                    </td>
